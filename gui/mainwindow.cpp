    ...
    : QMainWindow(parent)
{
    ...
    // Carica il file aperto all'ultima chiusura
    loadLastFile();
}
...
void MainWindow::closeEvent(QCloseEvent* event) {
    // Salva il percorso del file attualmente aperto
    QSettings settings("GestioneAttivita", "GestioneAttivita");
    if (!fileManager->currentFilePath().isEmpty()) {
        settings.setValue("lastOpenedFile", fileManager->currentFilePath());
    }
    event->accept();
}
...
void MainWindow::loadLastFile() {
    ...
    // Verifica che il file esista e sia un file valido
    if (!lastFile.isEmpty() && QFileInfo(lastFile).exists()) {
        // Carica il file senza mostrare il dialogo di apertura
        try {
            JsonRepo jsonRepo(lastFile);
            fileManager->setCurrentFilePath(lastFile);
            fileManager->setIsLoading(true);
            repo->carica(jsonRepo);
            fileManager->setIsLoading(false);
            clearSelectionAndDetails();
            statusBar()->showMessage("File caricato: " + lastFile, 3000);
        } catch (...) {
            // Se il caricamento fallisce, ignora silenziosamente
            fileManager->setIsLoading(false);
            qWarning() << "Impossibile caricare l'ultimo file:" << lastFile;
        }
    }
}
